...
int main(int argc, char **argv){
  ...
  if(my_rank == 0){
    ...
    MPI_Sendrecv( &sendbuf, strlen(sendbuf)+1, MPI_CHAR,  1, tag, 
                  &recvbuf, strlen(recvbuf)+1, MPI_CHAR, 1, tag,
    ...
    strcpy(recvbuf, "Hello");
    strcpy(sendbuf, "Hello");
    MPI_Sendrecv( &sendbuf, strlen(sendbuf)+1, MPI_CHAR,  0, tag, 
                  &recvbuf, strlen(recvbuf)+1, MPI_CHAR, 0, tag,
                  MPI_COMM_WORLD, &status);
